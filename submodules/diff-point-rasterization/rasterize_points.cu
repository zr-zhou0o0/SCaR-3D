...
// 这是从python直接调用的。加了一个返回的tensor
std::tuple<int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
           torch::Tensor, torch::Tensor, torch::Tensor> // ADD one more tensor
RasterizeGaussiansCUDA(
  ...
  torch::Tensor out_mean2D = torch::empty({P, 2}, float_opts); // <<< NEW: Tensor for mean2D output
  // 一个需要检查的地方：mean2D的size是（P,2），而不是（P,3）吗？
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        // out_mean2D.contiguous().data_ptr<float>(), // <<< PASS pointer to new tensor
        out_mean2D.contiguous().data<float>(), // <<< PASS data to new tensor???
        radii.contiguous().data<int>(),
        debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, radii, geomBuffer,
                         binningBuffer, imgBuffer, out_mean2D);
}
