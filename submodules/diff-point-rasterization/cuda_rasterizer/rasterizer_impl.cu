...
// of Gaussians.
// ADD(1) add out_means2D
// 这就是forward的实现过程，要确保out means 2d被正确填充。
int CudaRasterizer::Rasterizer::forward(
    ...
    float tan_fovy, const bool prefiltered, float *out_color, float *out_depth, float *out_means2D,
  ...
  // to RGB)
  // How to get the means2D???
  // out depth out color都是在render的时候给的，但是out means2D 在preprocess就应该填充了。
  // 所以到底是什么时候？
  // 这里就调用了forward.h / cu 文件里面的两个函数，preprocess和render
  // preprocess没有改动，但是理论上它的means2D是填进去了的。
  ...
  // Copy means2D data if P > 0 and out_means2D is provided
  // ADD(1)
  // 这时候preprocess填完就可以复制过来了。
  // render之后会改变它吗？不会的话我在render之后复制？
  if (P > 0 && out_means2D != nullptr)
  {
      CHECK_CUDA(cudaMemcpy(out_means2D, geomState.means2D, P * sizeof(float2), cudaMemcpyDeviceToDevice), debug);
  }
